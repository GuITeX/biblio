...
file/.style = {rectangle, rounded corners, draw, fill=red!30,
...
% nodes in chain                        
\node [file] (source) {\footnotesize File\\\footnotesize sorgente};
...
\node [file, right of=style, node distance=8em] (bib) {\footnotesize Database\\\footnotesize bibliografico};
...
\node [file, below of=bib, node distance=12em] (log1) {File di log};
...
\node [file , below of=pdflatex3 , node  distance=5em] (pdf) {File PDF};
\coordinate (middle4) at ($(biblatex.south)!0.5!(style.south)$);
\node [file , below of=middle4 , node  distance=17.7em] (log2) {File di log};
